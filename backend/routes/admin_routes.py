...
def get_all_records():
    ...
    for order in orders:
        ...
        order_info = {
            ...
            "customer_last_order": (
                customer.last_order.strftime("%Y-%m-%d %H:%M:%S")
...
def get_item_from_order(order):
    ...
    for sub_order in order.sub_orders:
        ...
        ingredient_details = [
            {"name": ingredient.ingredient.name, "action": ingredient.action}
        ...
        if status == "Delivered":
            for driver_id in driver_ids:
                ...
                if not driver:
                    return (
                        jsonify(
                            {"error": f"Delivery driver with ID {driver_id} not found"}
                        ),
                        404,
                    )
...
def get_all_drivers():
    ...
    for driver in drivers:
        driver_info = {
            ...
            "last_delivery": (
                driver.last_delivery.strftime("%Y-%m-%d %H:%M:%S")
                if driver.last_delivery
                else None
            ),
        }
...
def add_driver():
    ...
    return (
        jsonify({"message": "Delivery driver added", "driver_id": new_driver.id}),
        201,
    )
