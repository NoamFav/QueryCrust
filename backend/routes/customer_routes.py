...
def register():
    ...
    if not driver:
        ...
        db.session.commit()
        print(f"A new driver appeared from thin air for delivery area: {new_user.address}") # I'm not crazy, you're crazy
...
def get_menu():
    ...
    if category:
        items = Menu.query.filter_by(category=category.lower()).all() # Filter by category
    else:
    ...
    try:
        ...
        def find_or_create_driver(item_quantity=0):
            ...
            driver = (
                ...
                .order_by(func.count(Delivery.id).desc())  # Create more batches if possible
                .first()
            )
        ...
        for cart_item in cart.items:
            ...
            while item_quantity > 0:
                ...
                if is_pizza:
                    ...
                    delivery.pizza_count += pizzas_to_add # Increase the pizza count for the delivery
        ...
        return jsonify({
            ...
            'ordered_at': new_order.ordered_at.strftime('%Y-%m-%d %H:%M:%S'), # Format the date and time as a string like 'YYYY-MM-DD HH:MM:SS'
            'delivery_eta': new_order.delivery_eta.strftime('%Y-%m-%d %H:%M:%S'), # Format the delivery ETA as a string like 'YYYY-MM-DD HH:MM:SS'
...
def get_order():
    ...
    for order in orders:
        ...
        order_list.append({
            ...
            'ordered_at': order.ordered_at.strftime('%Y-%m-%d %H:%M:%S'), # Format the ordered date and time as a string like 'YYYY-MM-DD HH:MM:SS'
            'delivery_eta': order.delivery_eta.strftime('%Y-%m-%d %H:%M:%S'), # Format the delivery ETA as a string like 'YYYY-MM-DD HH:MM:SS'
...
def get_order_details(order_id):
    ...
    return jsonify({
        ...
        'total_cost': order.total_cost,
        'ordered_at': order.ordered_at.strftime('%Y-%m-%d %H:%M:%S'), # Format the ordered date and time as a string like 'YYYY-MM-DD HH:MM:SS'
        'delivery_eta': order.delivery_eta.strftime('%Y-%m-%d %H:%M:%S') if order.delivery_eta else None, # Format the delivery ETA as a string like 'YYYY-MM-DD HH:MM:SS'
        'status': order.status,
    ...
    cart_items = [
        {
            ...
            'customizations': item.customizations if item.customizations else [], # Get the customizations for the item
    ...
    try:
        ...
        if cart_item and cart_item.customizations == customizations:
            ...
            cart_item = CartItem(
                ...
                menu_id=menu_id,
                quantity=quantity,
                customizations=customizations if isinstance(customizations, list) else [], # Set the customizations for the item
